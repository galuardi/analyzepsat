...
#' @param sst.depth 
#' @param use.minmax 
#' @param ohc now with more content!
#' @param isotherm 
#'
#' @return
#' @export
#'
#' @examples
prepf2 <- function (tag, xmin = -100, xmax = 0, ymin = 10, ymax = 55, keepall = F, 
									 sst.depth = NULL, use.minmax = F, ohc = F, isotherm = 18) 
{
	dim1 = dim(tag$T)[1]
	dim2 = dim(tag$T)[2]
	Temp = tag$T[1:(dim1), ]
	...
	dat = as.data.frame(cbind(dates, (locs[, 4:5]), maxt[1:len], maxz[1:len]))
	names(dat)[4:7] = c("Lon", "Lat", "SST", "maxD")
	if(ohc ==T){
		cp = 3.993
		rho = 1025
		tag$ohc = cp * rho * apply(tag$T-isotherm, 1, sum, na.rm = T)/10000
		tag$ohc[tag$ohc<=0] = NA	
		dat = as.data.frame(cbind(dates, (locs[, 4:5]), tag$ohc[1:len], maxz[1:len]))
		names(dat)[4:7] = c("Lon", "Lat", "OHC", "maxD")
	}
